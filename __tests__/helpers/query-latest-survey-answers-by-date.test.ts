...
describe('Query Latest Survey Answers By Date', () => {
    ...
    describe('Date Filtering', () => {
        ...
        it('Should ignore survey answers that are too recent.', async () => {
            queryAllSurveyAnswersMock.mockResolvedValue([{
                surveyResultID: uuid().toString(), surveyName: 'Survey Name', date: formatISO(endDate), answers: ['5'], event: getDayKey(add(endDate, { seconds: -1 }))
            }]);
        ...
        it('Should ignore survey answers that are too recent (with events as dates).', async () => {
            queryAllSurveyAnswersMock.mockResolvedValue([{
                surveyResultID: uuid().toString(), surveyName: 'Survey Name', date: formatISO(add(endDate, { seconds: -1 })), answers: ['5'], event: getDayKey(endDate)
            }]);
    ...
    describe('Survey Answer Collation', () => {
        ...
        const oneDayBeforeEndDate = add(endDate, { days: -1 });

        const surveyAnswers: Partial<SurveyAnswer>[] = [
            { surveyResultID: surveyResultID1, surveyName: 'Survey Name', date: formatISO(add(oneDayBeforeEndDate, { hours: 8 })), answers: ['5'], event: getDayKey(startDate) },
            { surveyResultID: surveyResultID2, surveyName: 'Survey Name', date: formatISO(add(oneDayBeforeEndDate, { hours: 14 })), answers: ['7'], event: getDayKey(startDate) },
            { surveyResultID: surveyResultID2, surveyName: 'Survey Name', date: formatISO(add(oneDayBeforeEndDate, { hours: 14 })), answers: ['6'], event: getDayKey(startDate) },
            { surveyResultID: surveyResultID1, surveyName: 'Survey Name', date: formatISO(add(oneDayBeforeEndDate, { hours: 8 })), answers: ['8'], event: getDayKey(startDate) },
            { surveyResultID: surveyResultID3, surveyName: 'Other Survey Name', date: formatISO(add(oneDayBeforeEndDate, { hours: 9 })), answers: ['1'], event: getDayKey(startDate) },
            { surveyResultID: surveyResultID4, surveyName: 'Other Survey Name', date: formatISO(add(oneDayBeforeEndDate, { hours: 15 })), answers: ['3'], event: getDayKey(startDate) },
            { surveyResultID: surveyResultID4, surveyName: 'Other Survey Name', date: formatISO(add(oneDayBeforeEndDate, { hours: 15 })), answers: ['2'], event: getDayKey(startDate) },
            { surveyResultID: surveyResultID3, surveyName: 'Other Survey Name', date: formatISO(add(oneDayBeforeEndDate, { hours: 9 })), answers: ['4'], event: getDayKey(startDate) }
        ];
        ...
        it('Should return all survey answers from the latest result for each survey on each date.', async () => {
            ...
            const sortedAnswers = latestSurveyAnswersByDate[getDayKey(oneDayBeforeEndDate)]?.map(sa => sa.answers[0]).sort() ?? [];
