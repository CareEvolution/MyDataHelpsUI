...
describe('Data Collection Helper Function Tests', () => {
    ...
    describe('canQuery', () => {
        describe('V1', () => {
            it('Project - Should return a result with v1 enabled with all types and v2 disabled.', async () => {
                ...
                const result = canQuery(combinedDataCollectionSettings, 'Project', 'AnyType');

                expect(result).toEqual({
                    v1: { enabled: true, types: ['AnyType'] },
                    v2: { enabled: false }
                });
            });
            ...
            describe.each([
                ...
                test.each([
                    ...
                    const result = canQuery(combinedDataCollectionSettings, queryNamespace, queryTypes);
    ...
    describe('hasV1Data', () => {
        ...
        it('Should reject with false when the query fails.', async () => {
            ...
            queryDeviceDataMock.mockRejectedValue(error);
        ...
        it('Should support single string types.', async () => {
            queryDeviceDataMock.mockResolvedValue({ deviceDataPoints: [{}] } as DeviceDataPointsPage);

            const result = await hasV1Data(namespace, type1);

            expect(result).toBe(true);

            expect(queryDeviceDataMock).toHaveBeenCalledTimes(1);
            expect(queryDeviceDataMock).toHaveBeenCalledWith({ namespace: namespace, type: [type1], limit: 1 });
        });
    ...
    describe('hasV2Data', () => {
        ...
        it('Should support single string types.', async () => {
            ...
            const result = await hasV2Data(namespace, type1);

            expect(result).toBe(true);

            expect(queryDeviceDataV2Mock).toHaveBeenCalledTimes(1);
            expect(queryDeviceDataV2Mock).toHaveBeenCalledWith({ namespace: namespace, type: type1, limit: 1 });
        });
