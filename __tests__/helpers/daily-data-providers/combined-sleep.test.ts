    ...
    it("should return the maximum sleep for each day", async () => {
        ...
        queryDeviceData.mockImplementation((query: DeviceDataPointQuery) => {
            if (query.namespace === "Fitbit") {
                ...
                return Promise.resolve({
                    deviceDataPoints: [
                        buildGarminDataPoint("30600", "2023-01-01T12:02:00", "DurationInSeconds"),
                        buildGarminDataPoint("31980", "2023-01-02T13:01:02", "DurationInSeconds")
                    ]
                });
            }
        });
        queryDeviceDataV2.mockImplementation((query: DeviceDataV2Query) => {
            if (query.namespace === "Oura") {
                return Promise.resolve({
                    deviceDataPoints: [
                        buildOuraDataPoint("26000", "2023-01-02", "total_sleep_duration", "long_sleep"),
                        buildOuraDataPoint("25000", "2023-01-03", "total_sleep_duration", "long_sleep"),
                    ]
                });
            }
        })
...
function buildOuraDataPoint(value: string, startDate: string, pointType: string, sleepType: string): DeviceDataV2Point {
    let ddp = {
        id: "",
        participantID: "",
        ...
        insertedDate: "",
        modifiedDate: "",
        properties: { type: sleepType, day: startDate, total_sleep_duration: value }
...
function buildGarminDataPoint(value: string, startDate: string, type: string): DeviceDataPoint {
    let ddp: DeviceDataPoint = {
        id: "",
        namespace: "Garmin",
        insertedDate: "",
