...
export async function queryForDailyData(
    ...
    const dataPoints = await queryForDailyDataPoints(namespace, type, startDate, endDate, dateFn);
...
export async function queryForDailyDataPoints(
    ...
    return dataPoints.filter(dataPoint => !dateFn || dailyDataDateFilter(dateFn(dataPoint), startDate, endDate));
}
...
export async function queryForDailyDataV2(
    ...
    const dataPoints = await queryForDailyDataPointsV2(namespace, type, startDate, endDate, dateFn);
    return dataPoints.reduce((dailyData, dataPoint) => {
        const dayKey = getDayKey(dateFn(dataPoint)!);
        if (!dailyData[dayKey]) {
            dailyData[dayKey] = [];
...
export const buildMostRecentValueResult = (dailyData: DailyData, valueFn: DailyDataValueFunction = getFloatValue): DailyDataQueryResult => {
...
export const buildMaxValueResult = (dailyData: DailyData, valueFn: DailyDataValueFunction = getFloatValue): DailyDataQueryResult => {
    const result: DailyDataQueryResult = {};
...
export const buildTotalValueResult = (dailyData: DailyData, valueFn: DailyDataValueFunction = getFloatValue): DailyDataQueryResult => {
    const result: DailyDataQueryResult = {};
...
export const buildAverageValueResult = (dailyData: DailyData, valueFn: DailyDataValueFunction = getFloatValue): DailyDataQueryResult => {
    const result: DailyDataQueryResult = {};
