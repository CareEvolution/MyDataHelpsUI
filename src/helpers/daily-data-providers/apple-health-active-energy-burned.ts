...
export default async function(startDate: Date, endDate: Date, combinedDataCollectionSettings?: CombinedDataCollectionSettings): Promise<DailyDataQueryResult> {
    ...
    const queryCheckResult = canQuery(combinedDataCollectionSettings ?? await getCombinedDataCollectionSettings(true), 'AppleHealth', ['ActiveEnergyBurned', 'Active Energy Burned'], false);
