...
export default async function (startDate: Date, endDate: Date, combinedDataCollectionSettings?: CombinedDataCollectionSettings): Promise<DailyDataQueryResult> {
    ...
    if (canQuery(combinedDataCollectionSettings, 'Garmin', 'Calories').v1.enabled) {
        providers.push(garminActiveCaloriesDataProvider(startDate, endDate));
    }

    const appleHealthCanQueryResult = canQuery(combinedDataCollectionSettings, 'AppleHealth', ['ActiveEnergyBurned', 'Active Energy Burned']);
    ...
    if (canQuery(combinedDataCollectionSettings, 'HealthConnect', 'active-calories-burned-daily').v2.enabled) {
        providers.push(healthConnectActiveCaloriesBurnedDataProvider(startDate, endDate));
    }
    if (canQuery(combinedDataCollectionSettings, 'Oura', 'daily-activity').v2.enabled) {
