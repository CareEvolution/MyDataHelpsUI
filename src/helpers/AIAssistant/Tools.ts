...
export const SaveLastGraphTool = tool(
  async (input, config: LangGraphRunnableConfig): Promise<string> => {

    let participantId = config.configurable?.participantId;
    let lastGraph = await config.store?.get([participantId, "graphing"], "lastGraph");

    if (!lastGraph || !lastGraph.value || !lastGraph.value.image) return "Unable to find your last graph.";

    const binaryString = atob(lastGraph.value.image);
    ...
    return "Successfully saved your last graph to your files.";
  },
  {
    name: "saveLastGraph",
    description: "Save the last graph generated by the graphing tool.",
    schema: z.object({
      fileName: z.string().describe(`The name of the file to save. This should be descriptive but reasonably short, no more than 50 characters. It should
        end with the '.png' file extension.`),
    })
  }
);
