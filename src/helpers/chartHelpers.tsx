...
export function createLineChartDefs(
    ...
    return <defs>
        {series.map((s, i) => {
            let lineColor = colorOrDefault(s.color, "var(--mdhui-color-primary-mark)");
    ...
    return <defs>
        {series.map((s, i) =>
            <linearGradient key={`lg-${s.dataKey}`} id={`${gradientKey}${i}`} x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor={colorOrDefault(s.color, "var(--mdhui-color-primary-mark)")} stopOpacity={1.0} />
                <stop offset="100%" stopColor={colorOrDefault(s.color, "var(--mdhui-color-primary-mark)")} stopOpacity={0.7} />
            </linearGradient>
        )}
        {options?.thresholds?.map((threshold, index) =>
            <linearGradient key={`lg_thresh_${index}`} id={gradientKey + "_threshold" + index} x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor={colorOrDefault(threshold.overThresholdColor, "var(--mdhui-color-warning-mark)")} stopOpacity={1.0} />
                <stop offset="100%" stopColor={colorOrDefault(threshold.overThresholdColor, "var(--mdhui-color-warning-mark)")} stopOpacity={0.7} />
            </linearGradient>
        )}
    </defs>;
}
        ...
        {series.map((s, i) =>
            <>
                <linearGradient key={`lg-${s.dataKey}-fill`} id={`${gradientKey}${i}-fill`} x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor={colorOrDefault(s.areaColor, "var(--mdhui-color-primary-mark)")} stopOpacity={0.5} />
                    <stop offset="100%" stopColor={colorOrDefault(s.areaColor, "var(--mdhui-color-primary-mark)")} stopOpacity={0.2} />
                </linearGradient>
                <linearGradient key={`lg-${s.dataKey}-stroke`} id={`${gradientKey}${i}-stroke`} x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor={colorOrDefault(s.color, "var(--mdhui-color-primary-mark)")} />
                    <stop offset="100%" stopColor={colorOrDefault(s.color, "var(--mdhui-color-primary-mark)")} />
                </linearGradient>
            </>
        )}
    </defs>;
