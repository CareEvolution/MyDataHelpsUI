...
import { RotatingComponentCoordinator } from '../../../presentational';
import './AsthmaFacts.css';
...
export default function (props: AsthmaFactsProps) {
    ...
    return <RotatingComponentCoordinator interval="week" startDate={startOfYear(new Date())} innerRef={props.innerRef}>
        {facts.map((fact, index) => {
            return <div className="mdhui-asthma-fact" ref={props.innerRef} key={index}>
                {fact.title &&
                    <div className="mdhui-asthma-fact-title">{`Asthma Fact #${fact.title}`}</div>
                }
                <div className="mdhui-asthma-fact-text">{fact.text}</div>
            </div>;
        })}
