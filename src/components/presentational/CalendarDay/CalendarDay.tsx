...
export default function(props: CalendarDayProps) {
    ...
    const previousDayStates = computeStatesForDay(add(new Date(date), { days: -1 }));
    const currentDayStates = computeStatesForDay(date);
    const nextDayStates = computeStatesForDay(add(new Date(date), { days: 1 }));
    ...
    const combinedColors = (colorsToCombine: (ColorDefinition | undefined)[]): string => {
        const resolvedColorsToCombine = colorsToCombine.map(colorDefinition => {
    ...
    const currentDayStyle: CSSProperties | undefined = currentDayState ? {
    ...
    return <div ref={props.innerRef} className={dayClasses.join(' ')} style={dayStyle}>
        {props.onClick &&
            <UnstyledButton onClick={() => props.onClick!(date)}>
                <div className="mdhui-calendar-day-value" style={currentDayStyle}>{date.getDate()}</div>
            </UnstyledButton>
        }
        {!props.onClick &&
            <div className="mdhui-calendar-day-value" style={currentDayStyle}>{date.getDate()}</div>
