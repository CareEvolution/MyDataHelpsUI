...
export default function TimeSeriesChart(props: TimeSeriesChartProps) {
    ...
    if ( intervalType === "Dynamic" && !props.data?.length ) {
        intervalType = "Week";
    }
    ...
    const DayTick = ({ x, y, payload, index, width }: any) => {
        ...
        if (intervalType === "Month") {
            ...
            return <text fill="var(--mdhui-text-color-2)" x={adjustedX} y={y + 15} textAnchor={textAnchor} fontSize="12">{getFullDateString(currentDate)}</text>;
        }
        return <>
    ...
    function getXAxisTicks() {
        if (intervalType === "Dynamic") {
            ...
            if (!props.data || props.data.length === 0) {
                return [add(today, { days: -6 }).getTime(), today.getTime()];
            }
            ...
            return [firstDayWithData.getTime(), today.getTime()];
        }
