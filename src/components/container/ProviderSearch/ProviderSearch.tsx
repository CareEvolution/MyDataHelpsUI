...
export default function (props: ProviderSearchProps) {
  const [linkedExternalAccounts, setLinkedExternalAccounts] = useState<{
    [id: number]: ExternalAccount;
  }>({});
  ...
  function performSearch(search: string) {
    ...
    MyDataHelps.getExternalAccountProviders(
      search,
      props.providerCategories?.length === 1 ? props.providerCategories[0] : null,
      pageSize,
      currentPage
  ...
  function updateSearchResults(providers: ExternalAccountProvider[]) {
    setSearchResults((prevResults) =>
      prevResults.concat(providers.filter((a) => props.providerCategories?.indexOf(a.category) !== -1))
    );
  }
  ...
  function connectToProvider(provider: ExternalAccountProvider) {
    ...
    if (
      !props.previewState &&
      !(linkedExternalAccounts[providerID] && linkedExternalAccounts[providerID].status !== 'unauthorized')
    ) {
      MyDataHelps.connectExternalAccount(
        providerID,
        props.connectExternalAccountOptions || { openNewWindow: true }
      ).then(function () {
          ...
          searchResults.map((provider) => (
            ...
            >
              ...
              <div className="provider-info">
                ...
                {linkedExternalAccounts[provider.id] &&
                  linkedExternalAccounts[provider.id].status === 'unauthorized' && (
                    <div className="provider-status error-status">
                      {language('expired-reconnect')}
                    </div>
                  )}
                {linkedExternalAccounts[provider.id] &&
                  linkedExternalAccounts[provider.id].status !== 'unauthorized' && (
