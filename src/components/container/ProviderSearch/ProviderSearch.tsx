...
export default function ProviderSearch(props: ProviderSearchProps) {
    ...
    const [featuredProviders, setFeaturedProviders] = useState<ExternalAccountProvider[]>(props.featuredProviders ?? []);
    ...
    async function initialize() {
        ...
        if (!props.featuredProviders) {
            const featuredProvidersPromise: Promise<ExternalAccountProvider[]> = props.publicFeaturedProvidersApiUrl
                ? getFeaturedPublicProviders(props.publicFeaturedProvidersApiUrl, props.featuredProvidersContext)
                : MyDataHelps.getFeaturedProviders(props.featuredProvidersContext);
            setFeaturedProviders(await featuredProvidersPromise);
        }
    ...
    function updateSearchResults(newProviders: ExternalAccountProvider[]) {
        ...
        if (searchStringRef.current === "" && searchResults.length === 0 && featuredProviders.length > 0) {
