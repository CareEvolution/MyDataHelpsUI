...
export default function MealEditor(props: MealEditorProps) {
    ...
    const onAddItem = (): void => {
        ...
        const foodToAdd = itemsToAddInputRef.current.value.trim();
        if (!foodToAdd || mealToEdit.items?.some(foodItem => foodItem.name.toLowerCase() === foodToAdd.toLowerCase())) return;

        const updatedItems = [...(mealToEdit.items ?? []), { name: foodToAdd }];
        setMealToEdit({ ...mealToEdit, items: updatedItems });

        itemsToAddInputRef.current.value = '';
    };

    const onRemoveItem = (foodToRemove: string): void => {
        if (!mealToEdit.items || mealToEdit.items.length === 0) return;

        const updatedItems = mealToEdit.items.filter(foodItem => foodItem.name !== foodToRemove);
        setMealToEdit({ ...mealToEdit, items: updatedItems });
    };
